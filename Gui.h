...
{
private:
  HWND hWndStatusBar, hWndDrawArea, hWndSolidTiles, hWndMaskedTiles,
    hWndToolbar;
  HMENU hMenuPopup, hMenuP, hMenuView;
  HCURSOR hCursorDrawTile, hCursorStd, hCursorMove, hDACursor;
  int StatusBar_Height, MaskedTiles_Height, SolidTiles_Height,
    DrawArea_HeightOffset, Toolbar_Height;
  EDITMODE EditMode;
  long STPos, STPage;
...
public:
  ...
  void EnableCopyDelete(BOOL Enable)
  {
    ...
    EnableMenuItem(
      ...
      MF_BYCOMMAND | ((Enable) ? MF_ENABLED : MF_GRAYED));
    EnableMenuItem(
      hMenuPopup, ID_CUT, MF_BYCOMMAND | ((Enable) ? MF_ENABLED : MF_GRAYED));
    EnableMenuItem(
      hMenuPopup, ID_COPY, MF_BYCOMMAND | ((Enable) ? MF_ENABLED : MF_GRAYED));
    EnableMenuItem(
      hMenuPopup,
      ID_DELETE,
      MF_BYCOMMAND | ((Enable) ? MF_ENABLED : MF_GRAYED));
  ...
  {
    InitScrollBars(
      hWndDrawArea, Page.x, Page.y, LevelWidth - 1, LevelHeight - 1);
    RedrawLevel();
  }
  ...
  void EnablePaste(BOOL Enable)
  {
    EnableMenuItem(
      ...
      MF_BYCOMMAND | ((Enable) ? MF_ENABLED : MF_GRAYED));
    EnableMenuItem(
      hMenuPopup, ID_PASTE, MF_BYCOMMAND | ((Enable) ? MF_ENABLED : MF_GRAYED));
  ...
  void OnSize(WPARAM wParam, LPARAM lParam)
  {
    ...
    MoveWindow(
      ...
      Toolbar_Height,
      LOWORD(lParam),
      HIWORD(lParam) - DrawArea_HeightOffset,
      TRUE);
    SetWindowPos(
      hWndSolidTiles,
      NULL,
      0,
      HIWORD(lParam) - SolidTiles_Height - MaskedTiles_Height -
        StatusBar_Height,
      0,
      0,
    ...
    SetWindowPos(
      ...
      NULL,
      0,
      HIWORD(lParam) - MaskedTiles_Height - StatusBar_Height + 1,
      0,
      0,
      SWP_NOSIZE | SWP_NOZORDER);
  }
  ...
  void InitCZoneScrollbar(void)
  {
    InitScrollBars(
      hWndSolidTiles,
      0,
      STPage = 128 / TILESIZE_EDITOR,
      0,
      400 / TILESIZE_EDITOR - 1);
  }
  ...
  void SetStatusText(int p, char* Text)
  {
    SendMessage(hWndStatusBar, SB_SETTEXT, p, LPARAM(Text));
  }
    ...
    BOOL Actor)
  {
    EnableMenuItem(
      hMenuPopup,
      ID_DELETEACTOR,
      MF_BYCOMMAND | ((Actor) ? MF_ENABLED : MF_GRAYED));
    TrackPopupMenuEx(
      hMenuPopup,
  ...
  BOOL CheckViewMenuItem(UINT Id)
  {
    ...
    CheckMenuItem(
      hMenuView,
      Id,
      ((Mask & MF_CHECKED) ? MF_UNCHECKED : MF_CHECKED) | MF_BYCOMMAND);
  ...
  BOOL OnShowEditOps(void)
  {
          ...
          CheckMenuItem(hMenuView, ID_SHOWEDITOPS, ((Mask & MF_CHECKED) ?
  ...
  void OnShowActors(void)
  {
          ...
          CheckMenuItem(hMenuView, ID_SHOWACTORS, ((Mask & MF_CHECKED) ?
  MF_UNCHECKED : MF_CHECKED) | MF_BYCOMMAND);
  }
  ...
  void OnShowMaskedTiles(void)
  {
          ...
          CheckMenuItem(hMenuView, ID_SHOWMASKED, ((Mask & MF_CHECKED) ?
  MF_UNCHECKED : MF_CHECKED) | MF_BYCOMMAND);
  }*/

  void SetMoveCursor(BOOL MoveCursor)
  {
    SetCursor((MoveCursor) ? hCursorMove : hDACursor);
  }
} Gui;
