...
static BOOL		    D3D_Ready, FileChanged, ActorHovered, LButtonPressed, GrayedOutLastMasked;
...
// 					 Callback functions					 
// -------------------------------------------------------
LRESULT CALLBACK MainWindowProc(HWND hWndLocal, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_MOUSEMOVE:
	...
	case WM_ACTIVATE:
		if (WA_ACTIVE == LOWORD(wParam) || WA_CLICKACTIVE == LOWORD(wParam))
			Gui.EnablePaste(SelectCopyPaste.CheckClipboardDataAvailability(Gui.GetDrawAreaWindow()));
...
LRESULT CALLBACK DAWindowProc(HWND hWndLocal, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_NCMOUSEMOVE:
...
LRESULT CALLBACK TSWindowProc(HWND hWndLocal, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
	case WM_MOUSEMOVE:
	...
	case WM_LBUTTONUP:
		...
		ChangeEditMode((Gui.IsSolidTilesWin(hWndLocal)) ? EM_PLACESOLIDTILES : EM_PLACEMASKEDTILES);
		// fallthrough
...
void ChangeEditMode(EDITMODE Em)
{
	...
	if (EM_PLACESOLIDTILES == Em) BrushSize = BrushSizeSolid;
	else if (EM_PLACEMASKEDTILES == Em) BrushSize = BrushSizeMasked;
}
...
BOOL CheckIfFileWasSaved(void)
{
	...
	int Result = Message("This operation will discard any changes you made to the current file. Do you want to save it now?", MB_YESNOCANCEL | MB_ICONWARNING);
...
void NewFileOpened(string NewCurrentFileName, BOOL CallLevelSizeChanged)
{
	if (CallLevelSizeChanged)
	{
		...
		SendMessage(hWndEditOpsDlg, WM_COMMAND, MAKELONG(IDC_COMBOACTORTYPE, CBN_SELCHANGE), LPARAM(GetDlgItem(hWndEditOpsDlg, IDC_COMBOACTORTYPE)));
		Gui.RedrawCZone();
	...
	if (!CurrentFileName.size()) Str = "Unnamed - ";
	else Str = CurrentFileName.substr(CurrentFileName.rfind('\\')+1, CurrentFileName.size()) + " - ";
...
void CreateNameLists(void)
{
	...
	for (int i=0; i<CMP.GetNrFiles(); i++)
	{
		...
		else if (CMP.DetermineFileType(Names[i]) == TYPE_BGDROP && Names[i] != "STATUS.MNI")
...
void OnInitDialog(HWND hDlg, WPARAM wPAram, LPARAM lParam)
{
	...
	case IDD_CMPSPECIFY:
		...
		{
			char Str[32] = "";		
			DlgDirList(hDlg, Str, IDC_TREEDIRECTORYS, NULL, DDL_DIRECTORY | DDL_EXCLUSIVE);
		}*/
		break;
	
	case IDD_OPENFROMCMP:
		{
			for (list<string>::iterator it=LevelList.begin(); it != LevelList.end(); it++)
			{
				SendDlgItemMessage(hDlg, IDC_COMBOLEVELS, CB_ADDSTRING, 0, LPARAM((*it).c_str()));
			}
	...
	case IDD_EDITOPS:
		{
			...
			{
				SendDlgItemMessage(hDlg, IDC_COMBOACTORTYPE, CB_ADDSTRING, 0, LPARAM(ActorTypes[i]));
				i++;
			}
			...
			while (strcmp(ActorDefs[i].Name, "\0"))
			{
				if (strcmp(ActorDefs[i].Name, "??"))
					SendDlgItemMessage(hDlg, IDC_COMBOACTORS, CB_ADDSTRING, 0, LPARAM(ActorDefs[i].Name));
			...
			SendDlgItemMessage(hDlg, IDC_ACTORPREVIEW, STM_SETIMAGE, IMAGE_BITMAP, LPARAM(hBmActorPreview));
	...
	case IDD_LEVELINFO:
		...
		SetDlgItemInt(hDlg, IDC_TEXTNRTILES, Level.GetLevelWidth()*Level.GetLevelHeight(), FALSE);
		SetDlgItemInt(hDlg, IDC_TEXTNRSOLIDTILES, Level.GetNrSolidTiles(), FALSE);
		SetDlgItemInt(hDlg, IDC_TEXTNRMASKEDTILES, Level.GetNrMaskedTiles(), FALSE);
		...
		case IDC_COMBOACTORTYPE:
			{
				...
				int Type = SendDlgItemMessage(hDlg, IDC_COMBOACTORTYPE, CB_GETCURSEL, 0, 0);
				int i    = 0;
				
				while (strcmp(ActorDefs[i].Name, "\0"))
				{
					if (strcmp(ActorDefs[i].Name, "??") && (ActorDefs[i].Type == Type || Type == AT_ALL))
						SendDlgItemMessage(hDlg, IDC_COMBOACTORS, CB_ADDSTRING, 0, LPARAM(ActorDefs[i].Name));
		...
		case IDC_COMBOACTORS:
			{
				...
				while (strcmp(ActorDefs[ActorNr].Name, SelStr) && strcmp(ActorDefs[ActorNr].Name, "\0")) ActorNr++;
				...
				if (!strcmp(ActorDefs[ActorNr].ImageId, "-"))
				{
					...
					HBITMAP hBm = (HBITMAP)LoadImage(hInstance, MAKEINTRESOURCE(IDB_LEVELSYMBOLS), IMAGE_BITMAP, 0, 0, 0);
					...
					LoadMultipleActorImages((char*)ActorDefs[ActorNr].ImageId, &hBmActorPreview);
...
void OnCommand(HWND hWndCmd, WPARAM wParam, LPARAM lParam)
{
	...
	case ID_OPENFROMCMP:
		if (CheckIfFileWasSaved())
			DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_OPENFROMCMP), hWnd, DialogProc, IDD_OPENFROMCMP);
		break;

	case ID_UNDO:
		if (SelectCopyPaste.IsThereFloatingSel() && SelectCopyPaste.IsFloatingSelFromClipboard())
		{
	...
	case ID_PASTE:
		...
		SelectCopyPaste.Paste(Gui.GetDrawAreaWindow(), &Gui.GetScrollPos(), &LastHoveredTile, &Level);
		IndicateFileChange();
	...
	case ID_SELECTALL:
		{
			...
			ChangeEditMode(EM_SELECTDATA);
			SelectCopyPaste.Deselect(&Gui.GetScrollPos(), &Level);
			SelectCopyPaste.BeginFocusDragging(&Pt, &Gui.GetScrollPos(), Gui.GetDrawAreaWindow());
			...
			SelectCopyPaste.SelectionFinished(Gui.GetDrawAreaWindow(), &Gui.GetScrollPos(), &Level, NULL);
		}
		break;
	
	case ID_SHOWEDITOPS:
		ShowWindow(hWndEditOpsDlg, (Gui.CheckViewMenuItem(ID_SHOWEDITOPS)) ? SW_HIDE : SW_SHOW);
		break;
	...
	case ID_LEVELINFO:
		DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_LEVELINFO), hWnd, DialogProc, IDD_LEVELINFO);
		break;
	...
	case ID_SHOWMAPOVERVIEW:
		DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_LEVELOVERVIEW), hWnd, MapOverviewDlgProc, LPARAM(&Level));
		//ShowWindow(hWndLevelOverviewDlg, (Gui.CheckViewMenuItem(ID_SHOWMAPOVERVIEW)) ? SW_HIDE : SW_SHOW);
		break;
	...
	case ID_LEVELPROPS:
		...
		if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_LEVELPROPS), hWnd, LevelPropsDlgProc, LPARAM(&Params)))
		{		
			ChangeCZone(Level.GetCZone());
			IndicateFileChange();
		}
	...
	case ID_OPENFILE:
		...
		of.Flags = OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST | OFN_NOCHANGEDIR;
...
{
	//DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_CMPSPECIFY), hWnd, DialogProc, IDD_CMPSPECIFY);
...
void OnPaint(HWND hWndPaint, WPARAM wParam, LPARAM lParam)
{
	...
	if (D3D_Ready)
	{
		...
		if (DrawHoverRectangle && !ActorHovered && Gui.IsMouseInDrawArea() && Gui.GetEditMode() < EM_PLACEACTORS)
		{
...
void OnLButtonDown(HWND hWndButton, WPARAM wParam, LPARAM lParam)
{
	...
	POINT Pos = { LOWORD(lParam) / TILESIZE_EDITOR + Gui.GetScrollPos().x, HIWORD(lParam) / TILESIZE_EDITOR + Gui.GetScrollPos().y };
	
	if (Pos.x < 0 || Pos.y < 0 || Pos.x >= Gui.GetScrollPage().x+Gui.GetScrollPos().x || Pos.y >= Gui.GetScrollPage().y + Gui.GetScrollPos().y)
		return;
	...
	if (EM_SELECTDATA == Gui.GetEditMode())
	{
		...
		SelectCopyPaste.BeginFocusDragging(&MousePos, &Gui.GetScrollPos(), Gui.GetDrawAreaWindow());
	}
}
...
void OnLButtonUp(HWND hWndButton, WPARAM wParam, LPARAM lParam)
{
	...
	if (SelectCopyPaste.SelectionFinished(Gui.GetDrawAreaWindow(), &Gui.GetScrollPos(), &Level, &Pasted))
	{
	...
	if (SelectCopyPaste.IsSelectionActive()) return;
	...
	case EM_PLACESOLIDTILES:
		SetRect(&r, Pos.x, Pos.y, Pos.x + BrushSizeSolid.x, Pos.y + BrushSizeSolid.y);
		...
		for (y=0; y<BrushSizeSolid.y; y++, Pos.y++)
		{
			for (int x=0; x<BrushSizeSolid.x; x++, Pos.x++)
			{
				...
				if (CurrentSolidTile+y*40+x < 1000 && (CurrentSolidTile%40+x) < 40 && !(Cell.bMasked && Cell.Masked >= 128))
				{
					memcpy(&UndoCellsBefore[x + y*(r.right - r.left)], &Cell, sizeof(LevelCell));
					Level.SetCellAttributes(&Pos, Cell.Solid = CurrentSolidTile+y*40+x, (Cell.bMasked) ? Cell.Masked : 0, Cell.bMasked);
					memcpy(&UndoCellsAfter[x + y*(r.right - r.left)], &Cell, sizeof(LevelCell));
				}
			}
	...
	case EM_PLACEMASKEDTILES:
		SetRect(&r, Pos.x, Pos.y, Pos.x + BrushSizeMasked.x, Pos.y + BrushSizeMasked.y);
		UndoCellsBefore = new LevelCell[(r.right - r.left)*(r.bottom - r.top)];
		UndoCellsAfter = new LevelCell[(r.right - r.left)*(r.bottom - r.top)];
		...
		for (y=0; y<BrushSizeMasked.y; y++, Pos.y++)
		{
			for (int x=0; x<BrushSizeMasked.x; x++, Pos.x++)
			{
				...
				if (CurrentMaskedTile+y*40+x < 160 && (CurrentMaskedTile%40+x) < 40 && !(CurrentMaskedTile+y*40+x >= 128 && Cell.Solid))	
				{
					memcpy(&UndoCellsBefore[x + y*(r.right - r.left)], &Cell, sizeof(LevelCell));
					Level.SetCellAttributes(&Pos, Cell.Solid, Cell.Masked = ((wParam & MK_SHIFT) ? 0 : CurrentMaskedTile+y*40+x), Cell.bMasked = ((wParam & MK_SHIFT) ? FALSE : TRUE));	
					memcpy(&UndoCellsAfter[x + y*(r.right - r.left)], &Cell, sizeof(LevelCell));
				}
			}
...
BOOL OnContextMenu(HWND hWndContextMenu, WPARAM wParam, LPARAM lParam)
{
	...
	if (Pos.x < 0 || Pos.y < 0 || Pos.x >= Level.GetLevelWidth() || Pos.y >= Level.GetLevelHeight()) 
		return FALSE;
...
void OnMouseMove(HWND hWndMM, WPARAM wParam, LPARAM lParam)
{
	...
	POINT Pos = { MousePos.x/TILESIZE_EDITOR + Gui.GetScrollPos().x, MousePos.y/TILESIZE_EDITOR + Gui.GetScrollPos().y };

	if (Pos.x >= 0 && Pos.y >= 0 && Pos.x < Gui.GetScrollPage().x+Gui.GetScrollPos().x && Pos.y < Gui.GetScrollPage().y+Gui.GetScrollPos().y && Pos.x < Level.GetLevelWidth() && Pos.y < Level.GetLevelHeight())
	{
		LevelCell Cell;
		Level.GetCellAttributes(&Pos, &Cell);
		char StatusText[256];
		...
		if ((Pos.x != LastHoveredTile.x || Pos.y != LastHoveredTile.y))
		{
			...
			Gui.SetMoveCursor(SelectCopyPaste.PointInSelection(&MousePos, &Gui.GetScrollPos()));
			LastHoveredTile = Pos;
...
void InitMessages(void)
{
	...
	hWndEditOpsDlg = CreateDialogParam(hInstance, MAKEINTRESOURCE(IDD_EDITOPS), hWnd, DialogProc, IDD_EDITOPS);

	RECT r, r2;
	GetClientRect(hWnd, &r);
	GetWindowRect(hWndEditOpsDlg, &r2);
	SetWindowPos(hWndEditOpsDlg, NULL, r.right - (r2.right - r2.left) - 20, 40, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
}
