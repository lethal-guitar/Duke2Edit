...
void GuiClass::SizeStatusbarParts(void)
{
	...
	int Sizes[NR_STATUSBAR_PARTS] = { r.right-360, r.right-310, r.right-270, r.right-230, r.right };
...
BOOL GuiClass::Init(void)
{
	...
	hWndStatusBar = CreateStatusWindow(SBARS_SIZEGRIP | WS_CHILD | WS_VISIBLE, NULL, hWnd, STATUSBAR_ID);
	...
	TBButtons[3].fsStyle = TBButtons[7].fsStyle = TBButtons[10].fsStyle  
		                 = TBButtons[14].fsStyle = TBButtons[15].fsStyle 
						 = TBSTYLE_SEP;
	...
	hWndToolbar = CreateToolbarEx(hWnd, WS_CHILD | WS_VISIBLE | TBSTYLE_FLAT, TOOLBAR_ID, NR_TOOLBAR_BTNS+1, hInstance, IDR_MAINTOOLBAR, TBButtons, NR_TOOLBAR_BTNS, 16, 15, 24, 22, sizeof(TBBUTTON));
	...
	AdjustWindowRectEx(&r, WS_CHILD| WS_HSCROLL | WS_VSCROLL, FALSE, WS_EX_CLIENTEDGE);
	
	SolidTiles_Height = r.bottom - r.top;

	DrawArea_HeightOffset = StatusBar_Height + MaskedTiles_Height + SolidTiles_Height + Toolbar_Height + 1;

	GetClientRect(hWnd, &r);
	
	hWndDrawArea = CreateWindowEx(WS_EX_CLIENTEDGE, 
								  DRAWAREA_WNDCLASSNAME, 
								  NULL, 
								  WS_HSCROLL | WS_VSCROLL | WS_CHILD | WS_VISIBLE, 
								  0, Toolbar_Height, 
								  r.right, r.bottom - DrawArea_HeightOffset, 
								  hWnd, 
								  NULL, 
								  hInstance, 
								  NULL); 
	...
	if (!RegisterClassEx(&wc))
	{
		Error("GuiClass::Init(): Failed to register solid tile selection window class!");
		return FALSE;
	}
	...
	hWndSolidTiles = CreateWindowEx(WS_EX_CLIENTEDGE,
									SOLIDTILES_WNDCLASSNAME,
									"Solid Tiles",
									WS_CHILD | WS_VISIBLE | WS_VSCROLL,
									0, r2.bottom - StatusBar_Height - MaskedTiles_Height - SolidTiles_Height,
	...
	if (!RegisterClassEx(&wc))
	{
		Error("GuiClass::Init(): Failed to register masked tile selection window class!");
		return FALSE;
	}
	...
	hWndMaskedTiles = CreateWindowEx(WS_EX_CLIENTEDGE,
									MASKEDTILES_WNDCLASSNAME,
									"Masked Tiles",
									WS_CHILD | WS_VISIBLE,
									0, r2.bottom - StatusBar_Height - MaskedTiles_Height + 1,
									r.right - r.left, r.bottom - r.top,
									hWnd,
									NULL,
									hInstance,
	...
	InitScrollBars(hWndDrawArea, DAClientRect.right / TILESIZE_EDITOR, DAClientRect.bottom / TILESIZE_EDITOR, START_LEVELWIDTH-1, LEVEL_DATA_WORDS/START_LEVELWIDTH-1);
	...
	hMenuPopup = GetSubMenu(hMenuP = LoadMenu(hInstance, MAKEINTRESOURCE(IDR_POPUP)), 0);

	CheckMenuRadioItem(hMenuPopup, ID_PLACESOLID, ID_SELECT, ID_PLACESOLID, MF_BYCOMMAND);
...
void GuiClass::InitScrollBars(HWND hWndSBar, UINT PageHorz, UINT PageVert, int MaxHorz, int MaxVert)
{
	ZeroMemory(&ScrollInfo, sizeof(SCROLLINFO));
	ScrollInfo.cbSize = sizeof(SCROLLINFO);
...
void GuiClass::SetEditMode(EDITMODE Em)
{
	...
	CheckMenuRadioItem(hMenuPopup, ID_PLACESOLID, ID_SELECT, ID_PLACESOLID+EditMode, MF_BYCOMMAND);
