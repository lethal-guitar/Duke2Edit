...
void TransferCurrentStateToMap(Duke2Map* Map)
{
	...
	for (int y=StatesIt->CellsPos.top; y<StatesIt->CellsPos.bottom; y++)
	{
		for (int x=StatesIt->CellsPos.left; x<StatesIt->CellsPos.right; x++)
		{
			...
			memcpy(&Cell, &(StatesIt->Cells[x-StatesIt->CellsPos.left + (y-StatesIt->CellsPos.top)*(StatesIt->CellsPos.right - StatesIt->CellsPos.left)]), sizeof(LevelCell));
...
{
	if (States.empty() || StatesIt == States.begin()) return FALSE;
...
{
	if (States.empty() || StatesIt == LastState) return FALSE;
...
void AddUndoState(RECT* CellsPos, LevelCell* Cells, BOOL Link)
{
	...
	State.Cells = new LevelCell[(State.CellsPos.right - State.CellsPos.left)*(State.CellsPos.bottom - State.CellsPos.top)];
	memcpy(State.Cells, Cells, sizeof(LevelCell)*(State.CellsPos.right - State.CellsPos.left)*(State.CellsPos.bottom - State.CellsPos.top));
