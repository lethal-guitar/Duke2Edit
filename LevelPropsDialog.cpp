...
{
	if (!IsDlgButtonChecked(hDlg, IDC_CHECKCHANGEBG) && Enable) return;
	...
	if (IsDlgButtonChecked(hDlg, IDC_CHECKPREVIEWCHGBG))
	{
		...
		if (Enable)	
		{
			...
			SendDlgItemMessage(hDlg, IDC_BMPPREVIEWBDROP, STM_SETIMAGE, WPARAM(IMAGE_BITMAP), LPARAM(hBmBackdrop));
		}
		else
		{
...
void OnInitDialog(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
	...
	if (Params->CreateNewLevel) SetWindowText(hDlg, "Set properties for new level");
	else SetWindowText(hDlg, "Change level properties");
	
	int i=0;
	
	for (list<string>::iterator it=Params->CZoneList->begin(); it != Params->CZoneList->end(); it++, i++)
	{
		SendDlgItemMessage(hDlg, IDC_COMBOCZONE, CB_ADDSTRING, 0, LPARAM((*it).c_str()));
		
		if (!Params->CreateNewLevel && !_stricmp((*it).c_str(), Params->Level->GetCZone()))
	...
	for (it=Params->BackdropList->begin(); it != Params->BackdropList->end(); it++, i++)
	{
		SendDlgItemMessage(hDlg, IDC_COMBOBDROP, CB_ADDSTRING, 0, LPARAM((*it).c_str()));
		
		if (!Params->CreateNewLevel && !_stricmp((*it).c_str(), Params->Level->GetBGDrop()))
	...
	for (it=Params->MusicList->begin(); it != Params->MusicList->end(); it++, i++)
	{
		SendDlgItemMessage(hDlg, IDC_COMBOMUSIC, CB_ADDSTRING, 0, LPARAM((*it).c_str()));
		
		if (!Params->CreateNewLevel && !_stricmp((*it).c_str(), Params->Level->GetMusic()))
	...
	SendDlgItemMessage(hDlg, IDC_BMPPREVIEWCZONE, STM_SETIMAGE, WPARAM(IMAGE_BITMAP), LPARAM(hBmCZone));
	...
	if (Flags & (LF_SCROLLHORZVERT | LF_SCROLLHORZ | LF_AUTOSCROLLHORZ | LF_AUTOSCROLLVERT))
	{
	...
	if (Flags & LF_SCROLLHORZVERT || !(Flags & LF_SCROLLHORZVERT) && !(Flags & LF_SCROLLHORZ) && !(Flags & LF_AUTOSCROLLHORZ) && !(Flags & LF_AUTOSCROLLVERT))
	...
	for (it = Params->BackdropList->begin(); it != Params->BackdropList->end(); it++, i++)
	{
		SendDlgItemMessage(hDlg, IDC_COMBOCHGBG, CB_ADDSTRING, 0, LPARAM((*it).c_str()));
		
		if (!Params->CreateNewLevel && Flags & (LF_CHANGEBG1 | LF_CHANGEBG2) && atoi((*it).substr(4, 2).c_str()) == HIBYTE(Flags))
			SendDlgItemMessage(hDlg, IDC_COMBOCHGBG, CB_SETCURSEL, i, 0);
	}
	
	if (Flags & (LF_CHANGEBG1 | LF_CHANGEBG2) && !(Flags & LF_AUTOSCROLLHORZ) && !(Flags & LF_AUTOSCROLLVERT))
	{
	...
	if (Flags & LF_CHANGEBG1 || !(Flags & LF_CHANGEBG1) && !(Flags & LF_CHANGEBG2))
	...
	for (i=0; VALID_LEVEL_WIDTHS[i]; i++)
	{
		...
		if (!Params->CreateNewLevel && VALID_LEVEL_WIDTHS[i] == Params->Level->GetLevelWidth())
		{
			SendDlgItemMessage(hDlg, IDC_COMBOLEVELWIDTH, CB_SETCURSEL, i, 0);
			SetDlgItemInt(hDlg, IDC_TEXTLVLHEIGHT, LEVEL_DATA_WORDS / VALID_LEVEL_WIDTHS[i], TRUE);
		}
		else if (Params->CreateNewLevel && VALID_LEVEL_WIDTHS[i] == START_LEVELWIDTH)
		{
			SendDlgItemMessage(hDlg, IDC_COMBOLEVELWIDTH, CB_SETCURSEL, i, 0);
			SetDlgItemInt(hDlg, IDC_TEXTLVLHEIGHT, LEVEL_DATA_WORDS / VALID_LEVEL_WIDTHS[i], TRUE);
		}
...
WORD ReadFlagsFromDialog(HWND hDlg)
{
	...
	{
		if (IsDlgButtonChecked(hDlg, IDC_RADIOSCROLL1)) Flags |= LF_SCROLLHORZVERT;
		else if (IsDlgButtonChecked(hDlg, IDC_RADIOSCROLL2)) Flags |= LF_SCROLLHORZ;
		else if (IsDlgButtonChecked(hDlg, IDC_RADIOSCROLL3)) Flags |= LF_AUTOSCROLLHORZ;
		else if (IsDlgButtonChecked(hDlg, IDC_RADIOSCROLL4)) Flags |= LF_AUTOSCROLLVERT;
	}
	...
	{
		if (IsDlgButtonChecked(hDlg, IDC_RADIOCHGBG1)) Flags |= LF_CHANGEBG1;
		else if (IsDlgButtonChecked(hDlg, IDC_RADIOCHGBG2)) Flags |= LF_CHANGEBG2;
...
void OnCommand(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
	...
	case IDOK:
		...
		{
			if (IDYES == Message("Warning: Changing the size of the level will truncate a part of your level! Proceed?", MB_YESNO | MB_ICONQUESTION))
			{
		...
		{
			SelectCopyPaste.Deselect(&Gui.GetScrollPos(), Params->Level);
			Params->Level->Create(w, CZone, Music, BDrop, ReadFlagsFromDialog(hDlg));
		}
	...
	case CBN_SELCHANGE:
		...
		case IDC_COMBOLEVELWIDTH:
			SetDlgItemInt(hDlg, IDC_TEXTLVLHEIGHT, LEVEL_DATA_WORDS / VALID_LEVEL_WIDTHS[SendDlgItemMessage(hDlg, IDC_COMBOLEVELWIDTH, CB_GETCURSEL, 0, 0)], TRUE);
			break;
		}
		break;
	...
	case BN_CLICKED:
		...
		case IDC_CHECKBGSCROLL:
			if (IsDlgButtonChecked(hDlg, IDC_CHECKBGSCROLL))
			{
				...
				if (IsDlgButtonChecked(hDlg, IDC_RADIOSCROLL3) || IsDlgButtonChecked(hDlg, IDC_RADIOSCROLL4))
				{
					EnableWindow(GetDlgItem(hDlg, IDC_CHECKCHANGEBG), FALSE);
					EnableBGChange(hDlg, FALSE);
				}
