...
void EGASimulator::LoadPalette(PALENTRY* pPal)
{
	if (WriteInProgress) 
	{
		Error("EGASimulator::LoadPalette(): Write is in progress, can't change palette now!");
		return;
	}
	...
	{
		Error("EGASimulator::LoadPalette(): Exception raised during palette copy, copy failed! Original palette restored!");
		return;
	}
...
void EGASimulator::LoadPaletteFromFile(char* FileName)
{
	if (WriteInProgress) 
	{
		Error("EGASimulator::LoadPaletteFromFile(): Write is in progress, can't change palette now!");
		return;
	}
	...
	if (!ReadFile(hFile, Pal, sizeof(PALENTRY)*16, &NrRead, NULL) || NrRead != sizeof(PALENTRY)*16)
	{
		Error("EGASimulator::LoadPaletteFromFile(): Reading from source file failed!");
		CloseHandle(hFile);
		return;
	}
...
void EGASimulator::WriteByte(int Position, BYTE Value)
{
	if (!WriteInProgress)
	{
		Error("EGASimulator::WriteByte(): BeginWrite() must be called before writing bytes!");
		return;
	}
...
void EGASimulator::TransferImage(DWORD* pRawDataOut, RECT* r)
{
	if (WriteInProgress) 
	{
		Error("EGASimulator::TransferImage(): Write is in progress, can't transfer image now!");
		return;
	}

	for (int y=0; y<ImageSize.y; y++)
		for (int x=0; x<ImageSize.x; x++)
			pRawDataOut[y*ImageSize.x+x] = (((Mask[y*ImageSize.x+x]) ? 0xFF : 0x00) << 24) + (Palette[Memory[y*ImageSize.x+x]].r << 16) + (Palette[Memory[y*ImageSize.x+x]].g << 8) + (Palette[Memory[y*ImageSize.x+x]].b);
...
void EGASimulator::TransferImage(HBITMAP* phBmOut, HDC hDC, RECT* pr)
{
	...
	if (!pr) SetRect(&r, 0, 0, ImageSize.x, ImageSize.y);
	else SetRect(&r, pr->left, pr->top, pr->right, pr->bottom);
	...
	*phBmOut = CreateDIBitmap(hDC, &bmi.bmih, CBM_INIT, Data, LPBITMAPINFO(&bmi), DIB_RGB_COLORS);
