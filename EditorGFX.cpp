...
static LPDIRECT3DSURFACE9 CZoneGFX, CZoneGridGFX, ActorSymbol, CZoneDrawSurface, GrayedOut;
...
BOOL DrawMasked = TRUE, DrawActors = TRUE, DrawTiles = TRUE, DrawGrid, DrawHoverRectangle = TRUE;
...
{
	RECT CellsPos = { SCP->FocusRect.left/TILESIZE_EDITOR, SCP->FocusRect.top/TILESIZE_EDITOR, SCP->FocusRect.right/TILESIZE_EDITOR, SCP->FocusRect.bottom/TILESIZE_EDITOR };
	//RECT CellsPos = { SCP->FloatingSelDst.left/TILESIZE_EDITOR, SCP->FloatingSelDst.top/TILESIZE_EDITOR, SCP->FloatingSelDst.right/TILESIZE_EDITOR, SCP->FloatingSelDst.bottom/TILESIZE_EDITOR };
	RECT rSrc, rDst;
	D3D.BeginSprite();

	OffsetRect(&CellsPos, -(ScrollPos->x - SCP->StartScrollPos.x), -(ScrollPos->y - SCP->StartScrollPos.y));
	...
	for (int y=CellsPos.top; y<CellsPos.bottom; y++)
	{
		for (int x=CellsPos.left; x<CellsPos.right; x++)
		{
			if (DrawTiles)
			{
				...
				TileIndexY = SCP->FloatingSelection[x-CellsPos.left + (y-CellsPos.top)*(CellsPos.right - CellsPos.left)].Solid / 40;
				TileIndexX = SCP->FloatingSelection[x-CellsPos.left + (y-CellsPos.top)*(CellsPos.right - CellsPos.left)].Solid % 40;
				...
				SetRect(&rDst, xp*TILESIZE_EDITOR, yp*TILESIZE_EDITOR, xp*TILESIZE_EDITOR+TILESIZE_EDITOR, yp*TILESIZE_EDITOR+TILESIZE_EDITOR);
				...
				if (DrawMasked && SCP->FloatingSelection[x-CellsPos.left + (y-CellsPos.top)*(CellsPos.right - CellsPos.left)].bMasked)
				{
					TileIndexY = 25 + SCP->FloatingSelection[x-CellsPos.left + (y-CellsPos.top)*(CellsPos.right - CellsPos.left)].Masked / 40;
					TileIndexX = SCP->FloatingSelection[x-CellsPos.left + (y-CellsPos.top)*(CellsPos.right - CellsPos.left)].Masked % 40;
					...
					D3D.DrawSprite(CZoneTex, &rSrc, &Scaling, NULL, 0, &Translation, 0xFFFFFFFF);
				}
			}

			SetRect(&rSrc, 0, 0, 32, 32);

			if (SCP->FloatingSelection[x-CellsPos.left + (y-CellsPos.top)*(CellsPos.right - CellsPos.left)].Actor && DrawActors)
	...
	SetRect(&CellsPos, CellsPos.left*TILESIZE_EDITOR, CellsPos.top*TILESIZE_EDITOR, CellsPos.right*TILESIZE_EDITOR, CellsPos.bottom*TILESIZE_EDITOR);
	
	D3D.BeginLine();
	
	D3DXVECTOR2 Points[2] = { D3DXVECTOR2(CellsPos.left, CellsPos.top), D3DXVECTOR2(CellsPos.left, CellsPos.bottom) };
...
void DrawMap(POINT* ScrollPos, POINT* pPage, POINT* SelectedTile)
{
	...
	if (Page.x >= Level.LevelWidth)  Page.x = Level.LevelWidth;
	if (Page.y >= Level.GetLevelHeight()) Page.y = Level.GetLevelHeight();
	...
	for (int y=ScrollPos->y; y<ScrollPos->y + Page.y; y++)
	{
		for (int x=ScrollPos->x; x<ScrollPos->x + Page.x; x++)
		{
			...
			if (SelectedTile->x+ScrollPos->x == x && SelectedTile->y+ScrollPos->y == y)
...
{
	RECT HoverRect = { HoverTile->x*TILESIZE_EDITOR, HoverTile->y*TILESIZE_EDITOR, BrushSize.x*TILESIZE_EDITOR, BrushSize.y*TILESIZE_EDITOR };
	HoverRect.right += HoverRect.left;
	HoverRect.bottom += HoverRect.top;
	
	D3D.BeginLine();

	D3DXVECTOR2 Points[2] = { D3DXVECTOR2(HoverRect.left, HoverRect.top), D3DXVECTOR2(HoverRect.left, HoverRect.bottom) };
...
void DrawSolidTiles(int Pos, int Page)
{
	...
	Rectangle(hDC, r.left, r.top, r.right - ((Offset.x > 0) ? Offset.x : 0), r.bottom - ((Offset.y > 0) ? Offset.y : 0));
	DeleteObject(hPen);

	CZoneDrawSurface->ReleaseDC(hDC);
	
	SetRect(&r, 0, Pos*TILESIZE_GAME, 320, Pos*TILESIZE_GAME+Page*TILESIZE_GAME);
...
void GetTileAttributesString(WORD Tile, char* Dst)
{
	...
	if (CZoneAttributes[Tile] & TA_SOLIDTOP || CZoneAttributes[Tile] & TA_SOLIDBOTTOM || CZoneAttributes[Tile] & TA_SOLIDLEFT || CZoneAttributes[Tile] & TA_SOLIDRIGHT)
	{
		strcat(Dst, "Solid ");
...
void CreateGridTex(POINT* ScrollPage)
{
	...
	while (!IsPowerOf2(Page.x*TILESIZE_EDITOR)) Page.x++;
	while (!IsPowerOf2(Page.y*TILESIZE_EDITOR)) Page.y++;
	...
	GridTex = D3D.CreateTexture("GridTex", Page.x*TILESIZE_EDITOR, Page.y*TILESIZE_EDITOR, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT);

	LPDIRECT3DTEXTURE9 Dummy = D3D.CreateTexture("Dummy", Page.x*TILESIZE_EDITOR, Page.y*TILESIZE_EDITOR, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM);
	...
	for (int y=0; y<Page.y*TILESIZE_EDITOR; y++)
		for (int x=0; x<Page.x*TILESIZE_EDITOR; x++)
			Pixels[x + y*Pitch] = (!(x % TILESIZE_EDITOR) || !(y % TILESIZE_EDITOR)) ? GRID_COLOR : 0;
...
{
	Level.Create(START_LEVELWIDTH, (char*)START_CZONE, (char*)START_MUSIC, (char*)START_BGDROP, START_FLAGS);	
	LoadCZone(START_CZONE);
	ActorSymbol = D3D.CreateSurfaceFromResource(hInstance, MAKEINTRESOURCE(IDB_LEVELSYMBOLS), 0, 0, D3DFMT_X8R8G8B8);
	CZoneDrawSurface = D3D.CreateSurface(320, 200, D3DFMT_X8R8G8B8, D3DPOOL_DEFAULT);
