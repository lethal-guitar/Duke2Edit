...
{
	int ImgSize = (ActorImages[Id].x/8)*(ActorImages[Id].y/8)*((ActorImages[Id].Masked) ? 40 : 36);
		...
		for (int Col=0; Col<ActorImages[Id].x/8; Col++)
		{
			for (int y=0; y<8; y++)
			{
				for (int Plane=0; Plane<5; Plane++)
				{
					...
					{
						EGA->SelectPlane(Plane - 1);
						EGA->WriteByte(Row*ActorImages[Id].x+y*(ActorImages[Id].x/8)+Col, Data[DataIndex]);
					}
					else
					{
						EGA->WriteMaskByte(Row*ActorImages[Id].x+y*(ActorImages[Id].x/8)+Col, Data[DataIndex]);
					}
...
BOOL LoadMultipleActorImages(char* Images, HBITMAP* Dst)
{
	...
	for (list<ImgDef>::iterator it = ImagesList.begin(); it != ImagesList.end(); it++)
	{
		if (it->Offset.x*8+it->Size.x > ResultSize.x) ResultSize.x = it->Offset.x*8+it->Size.x;
		if (it->Offset.y*8+it->Size.y > ResultSize.y) ResultSize.y = it->Offset.y*8+it->Size.y;
	}
	...
	{
		int MaxX = it->Size.x + it->Offset.x*8, MaxY = it->Size.y + it->Offset.y*8;

		for (int y=it->Offset.y*8; y<MaxY; y++)
		{
			for (int x=it->Offset.x*8; x<MaxX; x++)
			{
				int in = x-it->Offset.x*8 + (y-it->Offset.y*8)*(MaxX-it->Offset.x*8);
		...
		//BitBlt(hDCMem, it->Offset.x*8, it->Offset.y*8, it->Size.x, it->Size.y, hDCMem2, 0, 0, SRCPAINT);
		SelectObject(hDCMem2, it->Bitmap);		
		BitBlt(hDCMem, it->Offset.x*8, it->Offset.y*8, it->Size.x, it->Size.y, hDCMem2, 0, 0, SRCCOPY);
		//}
		//BitBlt (hdc, 0, 0, 100, 100 , hdcPic, 100, 0, 0x220326) ;
		//BitBlt (hdc, 0, 0, 100, 100 , hdcPic, 0, 0, SRCPAINT) ;
		//int ret = MaskBlt(hDCMem, it->Offset.x*8, it->Offset.y*8, it->Size.x, it->Size.y, hDCMem2, 0, 0, it->Mask, 0, 0, MAKEROP4(SRCCOPY, DSTINVERT0x00AA0029));
	...
	*Dst = CreateDIBitmap(hDC, &bmi.bmih, CBM_INIT, DestData, LPBITMAPINFO(&bmi), DIB_RGB_COLORS);
...
BOOL LoadImageFromCMP(const char* Name, LPDIRECT3DSURFACE9* Dst, int dx, int dy)
{
	...
	LPDIRECT3DSURFACE9 Srf = D3D.CreateSurface(320, SizeY, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT);
	...
	if (dx || dy)
	{
		LPDIRECT3DSURFACE9 Tmp = D3D.CreateSurface(dx, dy, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT);
		D3D.StretchRect(Srf, Tmp);
		D3D.ReleaseSurface(Srf);
...
BOOL LoadImageFromCMP(const char* Name, LPDIRECT3DTEXTURE9* Dst)
{
	...
	LPDIRECT3DTEXTURE9 DstTex = D3D.CreateTexture(Name, 512*2, 256*2, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT);
	
	if (!DstTex)
		return FALSE;

	LPDIRECT3DTEXTURE9 Dummy = D3D.CreateTexture("Dummy", 512*2, 256*2, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM);
...
static BOOL LoadLevelToEditor(BYTE* Level, Duke2Map* DstMap)
{
	...
	DstMap->Create(LevelWidth, Header.CZone, Header.Music, Header.BGDrop, Header.Flags);
	...
	for (int i=0; i<NrActors; i++)
	{
		...
		if (Pos.x >= LevelWidth || Pos.y >= LEVEL_DATA_WORDS / LevelWidth)
			Error("LoadLevelToEditor(): Invalid Actor Position: (%d, %d)!", Pos.x, Pos.y);
		else
		...
		for (int x=0; x<DstMap->GetLevelWidth(); x++)
		{
			...
			else
			{
				...
				{								   // (solid tile #0 as background)
					DstMap->SetCellAttributes(&Pos, 0, ((CurrentCell>>3) - 1000) / 5, TRUE);
				}
				else // complex (mixed masked/solid) masked tile
				{
					...
					DstMap->SetCellAttributes(&Pos, LOBYTE(CurrentCell) + Shift*256, Tmp >> 2, TRUE);
				}
			}
		}
	}
	...
	while (i < ExtraInfoSize)
	{
		...
		{
			if (ExtraInfo[i] != 0)
				WriteLogFile("Warning: LoadCMPLevel(): Reached end of ExtraInfo before actual ending!\n");
			...
			for (int j=0; j<DataBytes; j++)
			{
				...
				for (int k=0; k<4; k++)
				{
					if (!DstMap->GetCellAttributes(&Pos, &Cell))
						Error("Error: LoadCMPLevel(): GetCellAttributes() failed during read-out of tile shifting information! Pos: %d, %d", Pos.x, Pos.y);

					Cell.Masked += ((Data & (0x03 << k*2)) >> k*2)*32;

					DstMap->SetCellAttributes(&Pos, Cell.Solid, Cell.Masked, Cell.bMasked);
		...
		if (ExtraInfo[i+1] != 0)
		{
			for (int j=0; j< ExtraInfo[i]; j++)
			{
				...
				{
					if (!DstMap->GetCellAttributes(&Pos, &Cell))
						Error("LoadCMPLevel(): GetCellAttributes() failed during read-out of tile shifting information! Pos: %d, %d", Pos.x, Pos.y);
...
static void GenerateTSI(Duke2Map* Level, list<BYTE>* TSIData)
{
	...
	for (int y=0; y<Level->GetLevelHeight(); y++)
	{
		...
		{
			//if (Level->GetLevelHeight()-1 == y && Level->GetLevelWidth() - x <= 3 && !EntrysCollected.empty())
			//	goto NextEntryBlock;
...
NextEntryBlock:
					if (!EntrysCollected.front().Count)
					{
						...
						for (list<TSIEntry>::iterator it = EntrysCollected.begin(); !(*it).Count && it != EntrysCollected.end(); it++, Diff++);
						
						if (it == EntrysCollected.end()) Diff = 0;
				...
				if (Entry.PackShifts())
				{
					...
					if (!LastEntry.PackShifts() && PushBackEntrys && !EntrysCollected.empty())
					{
					...
					if (!EntrysCollected.empty() && Entry.PackShifts() == EntrysCollected.back().PackShifts())
		...
		for (int x=0; x<Level->GetLevelWidth(); x++)
		{
			...
			if (Cell.bMasked)
			{
				if (!Cell.Solid)
					...
					Tile = 0x8000 | (((((Cell.Masked - 32*(Cell.Masked/32)) << 2) | (Cell.Solid/256)) << 8) | (Cell.Solid - 256*(Cell.Solid/256)));
			}
			else
				Tile = Cell.Solid << 3;
